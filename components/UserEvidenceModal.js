...
const UserEvidenceModal = ({ visible, onDismiss, onSubmit, initialData }) => {
    const isEditing = !!initialData;
    const [title, setTitle] = useState('');
    // ... (restante dos estados como no EvidenceModal.js) ...
    ...
    useEffect(() => {
        if (visible) {
            if (isEditing) {
                ...
                setAttachLocation(!initialData.location); // Se já tem loc, não anexa nova por padrão
            } else {
                setTitle('');
                setDescription('');
    ...
    const handlePickImage = async (useCamera = false) => { /* ... (lógica igual) ... */
        const permission = useCamera
            ? await ImagePicker.requestCameraPermissionsAsync()
            : await ImagePicker.requestMediaLibraryPermissionsAsync();
        if (!permission.granted) { Alert.alert("Permissão Negada", `É necessário permitir o acesso à ${useCamera ? 'câmera' : 'galeria'}.`); return; }
        const action = useCamera ? ImagePicker.launchCameraAsync : ImagePicker.launchImageLibraryAsync;
        let result = await action({ mediaTypes: ImagePicker.MediaType = 'images', allowsEditing: true, aspect: [4, 3], quality: 0.5, base64: true });
        if (!result.canceled) { setImageData({ uri: result.assets[0].uri, base64: result.assets[0].base64 }); }
    };

    const requestLocationAndSubmit = async () => {
        if (title.trim() === '') { Alert.alert("Campo Obrigatório", "O título é obrigatório."); return; }
        setLocationLoading(true);
        let locationPayload = null;
        if (attachLocation) {
            let { status } = await Location.requestForegroundPermissionsAsync();
            if (status !== 'granted') { Alert.alert('Permissão Negada', 'A evidência será salva sem georreferência.'); }
            else {
                try {
                    let location = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.High });
                    locationPayload = { type: 'Point', coordinates: [location.coords.longitude, location.coords.latitude] };
                } catch (error) { Alert.alert("Erro de Localização", "Não foi possível obter a localização."); setLocationLoading(false); return; }
            }
        }

        const formData = {
            title, description, evidenceType, category: category || undefined,
            data: evidenceType === 'image' ? (imageData?.base64 ? `data:image/jpeg;base64,${imageData.base64}` : undefined) : textData,
            location: locationPayload
        };
        if (isEditing && evidenceType === 'image' && !formData.data && initialData?.data) {
            formData.data = initialData.data; // Mantém a imagem antiga se não selecionou nova na edição
        } else if (isEditing && evidenceType === 'image' && !formData.data && !initialData?.data) {
            // Se estava editando imagem mas não tinha imagem e não adicionou nova, não envia data
            delete formData.data;
        } else if (isEditing && evidenceType !== 'image' && initialData?.data) {
            // Se mudou de imagem para outro tipo, remove o campo data se estiver vazio
            if (!formData.data || formData.data.trim() === '') delete formData.data;
        }
    ...
    const categoryOptions = [
        { label: 'Achados Periciais', value: 'achados_periciais' },
        { label: 'Dados Ante-mortem', value: 'dados_ante_mortem' },
        { label: 'Dados Post-mortem', value: 'dados_post_mortem' },
            ...
            <Modal visible={visible} onDismiss={onDismiss} contentContainerStyle={styles.modalContainer}>
                <ScrollView>
                    ...
                    <SegmentedButtons
                        ...
                        buttons={typeOptions.map(opt => ({ value: opt.value, label: opt.label }))} // Usando typeOptions
                        style={styles.input}
                    />
                    <SelectMenu
                        label="Categoria"
...
export default UserEvidenceModal;