...
import { Button, ActivityIndicator, Text, Card, Title, Divider } from 'react-native-paper';
import AsyncStorage from '@react-native-async-storage/async-storage';
import SelectMenu from '../../components/SelectMenu'; // Usando nosso componente reutilizável

const API_URL = 'https://odonto-legal-backend.onrender.com';

const AnaliseIAUserTab = ({ caseId }) => {
      ...
      <Card style={styles.card}>
        <Card.Content>
          ...
          <Text style={styles.description}>
            Use a IA para auxiliar na análise das evidências deste caso. Os resultados são sugestões e devem ser verificados por um perito.
          </Text>

          <SelectMenu
            label="Tipo de Análise"
            value={selectedAction?.key}
            options={aiActions.map(a => ({ label: a.label, value: a.key }))}
            onSelect={(key) => {
...
export default AnaliseIAUserTab;