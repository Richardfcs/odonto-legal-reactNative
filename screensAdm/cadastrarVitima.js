...
const CadastrarVitimaScreen = ({ route }) => {
  const navigation = useNavigation();
  const { caseId } = route.params;

  const [formData, setFormData] = useState({ /* Estado inicial completo */ });
  ...
  const onDateChange = (event, selectedDate) => {
    ...
    if (selectedDate) {
      if(datePickerTarget === 'death') setDateOfDeath(selectedDate);
      if(datePickerTarget === 'discovery') setDateOfDiscovery(selectedDate);
    }
  };
  ...
  const handleRegister = async () => {
    if (!formData.victimCode || !formData.identificationStatus) {
        Alert.alert("Erro", "Código da Vítima e Status de Identificação são obrigatórios.");
        return;
    }
    setLoading(true);
    const token = await AsyncStorage.getItem('token');
    
    // Constrói o payload com validação e limpeza
    let payload = {
        case: caseId,
        victimCode: formData.victimCode,
        identificationStatus: formData.identificationStatus,
        name: (formData.identificationStatus === 'nao_identificada') ? 'Desconhecida' : formData.name,
        // Converte para número ou undefined
        ageAtDeath: formData.ageAtDeath ? parseInt(formData.ageAtDeath, 10) : undefined,
        statureCm: formData.statureCm ? parseFloat(formData.statureCm) : undefined,
        // Objetos
        estimatedAgeRange: {
            min: formData.estimatedAgeMin ? parseInt(formData.estimatedAgeMin, 10) : undefined,
            max: formData.estimatedAgeMax ? parseInt(formData.estimatedAgeMax, 10) : undefined,
        },
        ...
        postMortemIntervalEstimate: { minHours: formData.postMortemMinHours ? parseInt(formData.postMortemMinHours, 10) : undefined, maxHours: formData.postMortemMaxHours ? parseInt(formData.postMortemMaxHours, 10) : undefined, estimationMethod: formData.postMortemEstimationMethod },
        ...
        dateOfDiscovery: dateOfDiscovery.toISOString().split('T')[0],
        // Campos que podem ser undefined se vazios
        gender: formData.gender,
        ethnicityRace: formData.ethnicityRace,
        bodyMassIndexCategory: formData.bodyMassIndexCategory,
        timeOfDayDiscovery: formData.timeOfDayDiscovery,
        mannerOfDeath: formData.mannerOfDeath,
        causeOfDeathPrimary: formData.causeOfDeathPrimary,
        dentalRecordStatus: formData.dentalRecordStatus,
        dentalRecordSource: formData.dentalRecordSource,
        additionalNotes: formData.additionalNotes,
        // Arrays de strings
        otherDistinctivePhysicalFeatures: formData.otherDistinctivePhysicalFeatures?.split(',').map(s => s.trim()).filter(Boolean),
        skeletalFeatures: formData.skeletalFeatures?.split(',').map(s => s.trim()).filter(Boolean),
        photosUrls: formData.photosUrls?.split(',').map(s => s.trim()).filter(Boolean),
    };
    
    // Limpeza final de chaves com valores undefined ou objetos vazios
    payload = JSON.parse(JSON.stringify(payload, (key, value) => {
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            ...
            for(const objKey in value) {
                if(value[objKey] !== undefined && value[objKey] !== '') {
                    cleanedObject[objKey] = value[objKey];
                }
            }
    ...
    try {
        ...
        if (!response.ok) throw new Error(result.error || result.message || "Erro ao cadastrar vítima.");
            ...
            <View style={styles.content}>
              ...
              <List.AccordionGroup expandedId={expandedAccordion} onAccordionPress={setExpandedAccordion}>
                  ...
                  <Card style={styles.card}><List.Accordion title="2. Dados Demográficos" id="2" titleStyle={styles.accordionTitle}><View style={styles.accordionContent}>
                      ...
                      <TextInput label="Estatura (cm)" value={formData.statureCm} onChangeText={v => handleInputChange('statureCm', v)} keyboardType="numeric" mode="outlined" style={styles.input} />
                  ...
                  <Card style={styles.card}><List.Accordion title="5. Dados Odontolegais" id="5" titleStyle={styles.accordionTitle}><View style={styles.accordionContent}>
                      ...
                      <TextInput label="Características Físicas (separar por vírgula)" value={formData.otherDistinctivePhysicalFeatures} onChangeText={v => handleInputChange('otherDistinctivePhysicalFeatures', v)} multiline mode="outlined" style={styles.input} />
                      <TextInput label="Características Esqueléticas (separar por vírgula)" value={formData.skeletalFeatures} onChangeText={v => handleInputChange('skeletalFeatures', v)} multiline mode="outlined" style={styles.input} />
                  </View></List.Accordion></Card>
                  ...
                  <Card style={styles.card}><List.Accordion title="6. Dados Forenses Adicionais" id="6" titleStyle={styles.accordionTitle}><View style={styles.accordionContent}>
                      ...
                      <View style={styles.row}><TextInput label="Mín. Horas" value={formData.postMortemMinHours} onChangeText={v => handleInputChange('postMortemMinHours', v)} keyboardType="numeric" mode="outlined" style={styles.halfInput} /><TextInput label="Máx. Horas" value={formData.postMortemMaxHours} onChangeText={v => handleInputChange('postMortemMaxHours', v)} keyboardType="numeric" mode="outlined" style={styles.halfInput} /></View>
                  ...
                  <Card style={styles.card}><List.Accordion title="7. Metadados" id="7" titleStyle={styles.accordionTitle}><View style={styles.accordionContent}>
                      <TextInput label="URLs de Fotos (separar por vírgula)" value={formData.photosUrls} onChangeText={v => handleInputChange('photosUrls', v)} multiline mode="outlined" style={styles.input} />
...
export default CadastrarVitimaScreen;